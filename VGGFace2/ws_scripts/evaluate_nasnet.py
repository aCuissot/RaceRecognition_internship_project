...
nas_model = Model(source_model.input, outS)
nas_model_multitask = nas_model
nas_model_multitask.summary()
# plot_model(nas_model_multitask, to_file=os.path.join(dirnm, 'NASNet50.png'), show_shapes=True)
...
if __name__ == '__main__':
    ...
    ckpntlist = ['nasnet.20-1.46.hdf5']
    # ckpntlist = [sys.argv[1]]
    nas_model_multitask.load_weights(dirnm + '/' + ckpntlist[0])
    from dataset_tools import load_for_pred
    from dataset_tools import evaluate_performance

    """ data, Y_true = load_for_pred(val_dataset, batch_size, shape)
    print(Y_true.shape)
    Y_pred = vgg_model_multitask.predict(data, batch_size, verbose=1) """
    Y_pred, Y_true = evaluate_performance(val_dataset, batch_size, shape, nas_model_multitask)
    print(Y_pred.shape)
    print(Y_true.shape)
